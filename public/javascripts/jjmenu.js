...
(function($) {
	...
	$.fn.jjmenu = function (clickEvent, param, myReplaces, effect) {
		...
		if ( clickEvent == "rightClick" || clickEvent == "both" ) {
			global.bind("mousedown", function(event) {
				if (event.button == 2 && (clickEvent == "rightClick" || clickEvent == "both")) {
					// // right click
		...
		if ( clickEvent == "click" || clickEvent == "both" ) {
			global.bind("click", function(event) {
		...
		function menu(id,param,myReplaces,el,effect) {
			...
			for (var i = 0; i < param.length; i++) {
				...
				if (param[i].get) {
					...
					for (var ii = 0; ii < uItems.length; ii++) {
						putItem(uItems[ii]);
					}
					checkPosition();
				} else {
			...
			function getEffect(id, effect) {
				...
				if (!effect.xposition)
					effect.xposition = "right";
				if (!effect.yposition)
					effect.yposition = "auto";
				if (!effect.orientation)
					effect.orientation = "auto";
